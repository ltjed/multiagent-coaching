...
class MultiAgentController(BaseController):
    ...
    def generate_shared_samples(self, steps, rand_prompts):
        ...
        if debug_enabled:
            logger.info(f"[DEBUG] generate_shared_samples() START - steps={steps}")
        ...
        if debug_enabled:
            logger.info(f"[DEBUG] world_size={world_size}")

        # Find a key with non-None value to calculate length
        # (DSBench batches may have label=None)
        length = None
        for key, data_list in rand_prompts.items():
            if data_list is not None:
                ...
                break
        if length is None:
            raise ValueError("All values in rand_prompts are None - cannot determine batch length")
        ...
        for key, data_list in rand_prompts.items():
            for i in range(world_size):
                ...
                # Handle None values (e.g., DSBench label=None case)
        ...
        if debug_enabled:
            logger.info(f"[DEBUG] Chunked {length} prompts into {world_size} chunks of size ~{chunk_size}")
        ...
        if debug_enabled:
            logger.info(f"[DEBUG] Submitted {len(all_refs)} remote generation tasks")
        ...
        if self.args.credit_model is None or steps <= self.args.warmup_steps_for_credit:
            if debug_enabled:
                logger.info(f"[DEBUG] BEFORE ray.get(all_refs) - waiting for {len(all_refs)} generation tasks")
            all_refs_results = ray.get(all_refs)
            if debug_enabled:
                logger.info(f"[DEBUG] AFTER ray.get(all_refs) - got {len(all_refs_results)} results")
            all_results = [r["sample"] for r in all_refs_results]

            # Extract coach metadata from ALL ranks and aggregate
            credit_status = None
            if len(all_refs_results) > 0:
                all_coach_metadata = [
                    r["coach_metadata"]
                    for r in all_refs_results
                    if "coach_metadata" in r
                ]

                if all_coach_metadata:
                    # Aggregate coach metadata across all ranks
                    aggregated_metadata = {
                        "coach_rewards": [],
                        "num_trajectories": 0,
                        "total_turns": 0,
                        ...
                        "rollout_correct": 0,
                        "rollout_total": 0,
                        "rollout_num_unique_prompts": 0
                    }

                    # Collect all coach rewards from all ranks
                    for metadata in all_coach_metadata:
                        aggregated_metadata["coach_rewards"].extend(metadata.get("coach_rewards", []))
                        aggregated_metadata["num_trajectories"] += metadata.get("num_trajectories", 0)
                        aggregated_metadata["total_turns"] += metadata.get("total_turns", 0)
                        aggregated_metadata["rollout_correct"] += metadata.get("rollout_correct", 0)
                        aggregated_metadata["rollout_total"] += metadata.get("rollout_total", 0)
                        # Track unique prompts if available (each rank processes different prompts)
                        if "rollout_samples_per_prompt" in metadata and metadata["rollout_samples_per_prompt"] > 0:
                            # Calculate num_unique_prompts from this rank: total_trajectories / samples_per_prompt
                            rank_total = metadata.get("rollout_total", 0)
                            rank_samples_per_prompt = metadata["rollout_samples_per_prompt"]
                            aggregated_metadata["rollout_num_unique_prompts"] += int(rank_total / rank_samples_per_prompt)

                    # Recompute rollout accuracy and samples_per_prompt from aggregated counts
                    if aggregated_metadata["rollout_total"] > 0:
                        aggregated_metadata["rollout_accuracy"] = (
                            aggregated_metadata["rollout_correct"] / aggregated_metadata["rollout_total"]
                        )

                    # Calculate samples_per_prompt from aggregated data
                    if aggregated_metadata["rollout_num_unique_prompts"] > 0:
                        aggregated_metadata["rollout_samples_per_prompt"] = (
                            aggregated_metadata["rollout_total"] / aggregated_metadata["rollout_num_unique_prompts"]
                        )
                    else:
                        aggregated_metadata["rollout_samples_per_prompt"] = 0

                    credit_status = self._convert_coach_metadata_to_wandb(aggregated_metadata)
                else:
                    credit_status = None
            else:
                credit_status = None
        ...
        if self.args.filter_agents_data:
            # FIX: Count actual samples, not Samples objects
            # The old code counted Samples objects (micro-batches), but the last batch
            # may have fewer samples (e.g., 29 samples = 7 full batches + 1 partial).
            # This caused NCCL deadlock when workers had different partial batch sizes.

            def count_actual_samples(samples_objects):
                """Count total actual samples across all Samples objects."""
                total = 0
                for s in samples_objects:
                    # Each Samples object has tensors with shape (num_samples, ...)
                    # Try multiple attributes for robustness
                    if hasattr(s, 'response_length') and s.response_length is not None:
                        total += len(s.response_length)
                    elif hasattr(s, 'sequences') and s.sequences is not None:
                        total += s.sequences.shape[0]
                    elif hasattr(s, 'labels') and s.labels is not None:
                        total += len(s.labels)
                return total

            # Count actual samples per agent per rank
            actual_samples_per_rank = [
                [count_actual_samples(rank_data[agent_id]) for agent_id in range(self.num_agents)]
                for rank_data in all_results
            ]

            # Find minimum actual samples across ranks for each agent
            min_actual_samples = [
                min(rank_samples[agent_id] for rank_samples in actual_samples_per_rank)
                for agent_id in range(self.num_agents)
            ]

            # Log for debugging
            print(f"[filter_agents_data] Actual samples per rank per agent: {actual_samples_per_rank}")
            print(f"[filter_agents_data] Min actual samples per agent: {min_actual_samples}")

            # Compute how many COMPLETE Samples objects (micro-batches) to keep
            # This discards partial batches to ensure all workers have equal sample counts
            num_complete_batches = [
                min_actual_samples[agent_id] // self.args.micro_rollout_batch_size
                for agent_id in range(self.num_agents)
            ]

            # Further reduce to ensure total samples divisible by train_batch_size
            # for clean gradient accumulation
            num_full_batches = [
                (num_complete_batches[agent_id] * self.args.micro_rollout_batch_size) // self.args.train_batch_size
                for agent_id in range(self.num_agents)
            ]
            ...
            print(f"[filter_agents_data] Keeping {num_elements_to_keep} complete Samples objects per agent")
        else:
            num_elements_to_keep = [
        ...
        if debug_enabled:
            logger.info(f"[DEBUG] generate_shared_samples() COMPLETE - returning sharded_data_refs")
    ...
    def clear_agent_kv_cache(self):
        """Clear KV cache from all vLLM engines to free GPU memory before training."""
        import ray
        for agent in self.agent_list:
            if agent.vllm_engines is not None:
                cache_reset_refs = []
                for engine in agent.vllm_engines:
                    cache_reset_refs.append(engine.reset_prefix_cache.remote())
                # Wait for all cache clearing to complete
                ray.get(cache_reset_refs)

    def clear_prefix_cache(self):
        """Override base class to use agent_list instead of agent."""
        self.clear_agent_kv_cache()

    def start_vllm_concurrency_logging(self, interval_seconds=60):
        """Start periodic concurrency logging on all agents' vLLM engines."""
        from marti.models.vllm.engine import start_concurrency_logging_all_engines

        for agent_idx, agent in enumerate(self.agent_list):
            if agent.vllm_engines is not None:
                print(f"[Agent {agent_idx}] Starting vLLM concurrency logging (every {interval_seconds}s)")
                start_concurrency_logging_all_engines(agent.vllm_engines, interval_seconds)

    def stop_vllm_concurrency_logging(self):
        """Stop periodic concurrency logging on all agents' vLLM engines."""
        from marti.models.vllm.engine import stop_concurrency_logging_all_engines

        for agent in self.agent_list:
            if agent.vllm_engines is not None:
                stop_concurrency_logging_all_engines(agent.vllm_engines)
    ...
    def _convert_coach_metadata_to_wandb(self, coach_metadata):
        """
        Convert coach metadata to W&B loggable format.

        Args:
            coach_metadata: Dict with keys:
                - coach_rewards: List of dicts with trajectory_id, turn_id, agent_role, reward
                - num_trajectories: int
                - total_turns: int

        Returns:
            Dict with individual coach rewards for W&B logging
        """
        if coach_metadata is None or "coach_rewards" not in coach_metadata:
            return {}

        coach_rewards = coach_metadata["coach_rewards"]
        wandb_dict = {}

        # Log each individual coach reward with a unique key
        for reward_info in coach_rewards:
            traj_id = reward_info["trajectory_id"]
            turn_id = reward_info["turn_id"]
            agent_role = reward_info["agent_role"]
            reward = reward_info["reward"]

            # Create unique key for this coach evaluation
            key = f"coach/{agent_role}/traj{traj_id}_turn{turn_id}"
            wandb_dict[key] = reward

        # Also log summary statistics per agent role
        role_rewards = {}
        for reward_info in coach_rewards:
            agent_role = reward_info["agent_role"]
            reward = reward_info["reward"]

            if agent_role not in role_rewards:
                role_rewards[agent_role] = []
            role_rewards[agent_role].append(reward)

        for role, rewards in role_rewards.items():
            wandb_dict[f"coach/{role}/mean"] = sum(rewards) / len(rewards)
            wandb_dict[f"coach/{role}/count"] = len(rewards)

        # Add rollout accuracy if available
        if "rollout_accuracy" in coach_metadata and coach_metadata["rollout_accuracy"] is not None:
            wandb_dict["rollout/accuracy"] = coach_metadata["rollout_accuracy"]
            wandb_dict["rollout/correct"] = coach_metadata["rollout_correct"]
            wandb_dict["rollout/total"] = coach_metadata["rollout_total"]

        # Store raw metadata for JSON export
        wandb_dict["_rollout_metadata"] = {
            "accuracy": coach_metadata.get("rollout_accuracy"),
            "correct": coach_metadata.get("rollout_correct"),
            "total": coach_metadata.get("rollout_total"),
            "samples_per_prompt": coach_metadata.get("rollout_samples_per_prompt", 0)
        }
    ...
    def step(self, rand_prompts, episode, steps, pbar):
        ...
        # Start concurrency logging on first step only
        if steps == 1:
            self.start_vllm_concurrency_logging()
        ...
        # Clear KV cache to free GPU memory before training
        # KV cache is rebuilt from scratch in next rollout, so safe to discard
        self.clear_agent_kv_cache()
