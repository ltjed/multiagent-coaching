...
class ActorPPOTrainer(PPOTrainer):
    def __init__(
        ...
        **kwargs,
        ...
        """
        # DEBUG & FIX: Set expandable_segments in Ray worker to reduce fragmentation
        current_conf = os.environ.get('PYTORCH_CUDA_ALLOC_CONF', 'NOT_SET')
        print(f"[DEBUG ActorPPOTrainer] PYTORCH_CUDA_ALLOC_CONF={current_conf}")
    ...
    def ppo_train(self, global_steps):
        ...
        if global_steps > self.freezing_actor_steps:
            ...
            if self.args.training_mode == "sft":
                ...
                # Clear ds_active_sub_modules to fix ZeRO-3 GatheredParameters assertion error
                # Reference: https://github.com/huggingface/trl/issues/2840#issuecomment-2662747485
...
class ActorModelRayActor(BasePPORole):
    ...
    def save_model(self):
        ...
        # Save each agent to its own subdirectory using rolename
        # e.g., "actor_agent_problem_solver" -> save_path/actor_agent_problem_solver/
        save_path_with_agent = os.path.join(args.save_path, self.rolename)

        # save model checkpoint after fitting on only rank0
        self.strategy.save_model(
            self.ema_model if args.enable_ema else self.actor,
            None,
            save_path_with_agent,
        )
