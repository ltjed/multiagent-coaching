...
class ExperienceMaker(ABC):
    ...
    def make_experience_list(self, all_samples: List[Samples], **generate_kwargs) -> List[Experience]:
        ...
        for experience, reward in zip(experiences, rewards):
            ...
            elif self.advantage_estimator in ["reinforce", "reinforce_baseline", "rloo", "group_norm", "reinforce_plus_plus"]:
                # REINFORCE-family algorithms: compute cumulative returns
                # For reinforce_plus_plus: returns include KL penalty, then global normalization applied later
        ...
        # Global batch normalization for REINFORCE++ and GRPO
        # This normalization is prompt-agnostic: collects ALL advantages from ALL experiences
        # in batch, computes single global mean/std, normalizes all together.
        # Note: For reinforce_plus_plus, this is the ONLY normalization (no per-prompt processing).
        #       For group_norm (GRPO), this comes AFTER per-prompt normalization.
        if self.advantage_estimator in ["group_norm", "reinforce_plus_plus"]:
            all_advantages = []
            all_action_masks = []
            ...
            # Global mean (across ALL advantages in batch)
            ...
            # Apply global normalization to each experience
            for exp in experiences:
    ...
    def process_experiences(self, experiences: List[Experience]) -> Tuple[List[Experience], List[torch.Tensor]]:
        ...
        if args.advantage_estimator == "rloo":
            ...
            # REINFORCE++ (arxiv:2501.03262): Global batch normalization
            # Key difference from group_norm:
            #   - No per-prompt processing (no reshape by n_samples_per_prompt)
            #   - Just return raw rewards for global normalization in make_experience_batch()
            #   - Works with any n_samples_per_prompt (including n=1, unlike group_norm)
            #   - Normalization is prompt-agnostic (treats all advantages equally)
    ...
    def make_experience(self, samples: Samples) -> Experience:
        ...
        # init log probs
        # DEBUG: Print tensor shapes before reference model forward
        print(f"[DEBUG make_experience] sequences.shape={sequences.shape}, packed_seq_lens={packed_seq_lens}")
