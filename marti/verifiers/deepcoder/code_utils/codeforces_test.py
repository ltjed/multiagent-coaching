...
def _execute_in_process(queue, program, which_type, method_name, debug):
    """
    This function executes in a separate process to isolate compilation and function retrieval.
    Results or exceptions are passed back via the queue.
    """
    try:
        # Dynamic import and compile module
        ...
        if debug:
            # Capture any exception and send via queue
...
def compile_and_get_func(program, which_type, method_name, timeout, debug):
    """
    Uses multiprocessing to compile code and get function in a separate process,
    to support reliable timeout in non-main thread environments.
    ...
    # Wait for process to complete, or until timeout
    process.join(timeout)

    # If process is still running, it timed out
    if process.is_alive():
        process.terminate()  # Terminate process
        process.join()       # Wait for process to fully terminate
        if debug:
    ...
    # Check if result is function or exception
...
# --- Helper functions and modified execution functions ---

def _execute_method_in_process(queue, method, inputs):
    """Execute the given method in a safe subprocess."""
    try:
        faulthandler.enable()
        ...
        faulthandler.disable()
        # Put exception in queue for main process to handle
        queue.put(e)
...
def execute_cb_code(method, inputs_list, outputs_list, timeout, early_stop=True, debug=True):
    """
    Safely execute a method in a separate process to test each input, with timeout.
    """
    reliability_guard()
    results = []
    debug_infos = {}
