...
def call_sandbox_api(sandbox_fusion_url: str, code: str, stdin: str, compile_timeout: int, run_timeout: int, language: str = "python", files: Optional[Dict[str, str]] = None, fetch_files: Optional[List[str]] = None) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
    """
    ...
    Args:
        ...
        files: Optional dict mapping filenames to base64-encoded content. Files will be
               available in the sandbox working directory during code execution.
               Example: {"train.csv": "base64_content", "test.csv": "base64_content"}
        fetch_files: Optional list of filenames to retrieve after execution. Files saved
               by the code will be returned as base64-encoded content in the response.
               Example: ["model.pkl", "preprocessor.pkl"]

    Returns:
        A tuple (response_json, error_message).
        If successful, response_json is the API's returned JSON object, error_message is None.
        The response includes a "files" dict with fetched file contents (base64-encoded).
    ...
    # Use provided files or default to empty dict (backward compatible)
    if files is None:
        files = {}

    # Use provided fetch_files or default to empty list (backward compatible)
    ...
    payload = json.dumps(
        {
            ...
            "files": files,  # Now accepts files for upload
